...
fig = px.bar(phase, x='Total New ASV', y='Stage', orientation='h', template='plotly_dark',
...
extras = [('Forecast Indicator','Pipeline por Forecast Indicator'),
...
grid_resp = AgGrid(disp, gridOptions=builder.build(), theme='streamlit-dark',
...
if sel_list:
    ...
    with st.expander(f"🗂 Ficha: {rec.get('Opportunity','')}", expanded=True):
        ...
        # Forecast Notes full width at end, label destacado
        notes = rec.get('Forecast Notes','')
        st.markdown("<hr/>", unsafe_allow_html=True)
        st.markdown(
            "<span style='color:#FFD700'><strong>Forecast Notes:</strong></span>",
            unsafe_allow_html=True
        )
        st.write(notes)
