...
# Carrega dados
import_path = load_data
# Execução

# Matched previous variable df
...
with st.sidebar.expander('Filtros adicionais'):
    ...
    if 'Pipeline Creator' in df.columns:
        ignore.append('Pipeline Creator')
...
r = df.groupby('Sales Team Member', as_index=False)['Total New ASV'].sum().sort_values('Total New ASV', ascending=False)
...
# Última seção: Dados Brutos (tudo disponível)
