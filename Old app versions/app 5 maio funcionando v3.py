...
# Totais principais (serão atualizados após seleção de vendedor)
# placeholder, substituído após carregamento de dados e seleção de membro
...
# Totais principais (baseado na seleção acima)
# Dataframe para cálculo de totais (considera filtros aplicados até aqui)
member_df = df.copy()
pipeline_stages = [
...
pipeline_total = member_df[member_df['Stage'].isin(pipeline_stages)]['Total New ASV'].sum()
won_total = member_df[member_df['Stage'].isin(won_stages)]['Total New ASV'].sum()
# Exibe totais antes dos gráficos
