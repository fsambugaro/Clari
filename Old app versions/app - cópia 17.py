...
# Sales Stage (com Closed desmarcados por padrão)
...
if 'Fiscal Quarter' in df.columns:
    quarters = sorted(df['Fiscal Quarter'].dropna().unique())
    sel_fq = st.sidebar.selectbox('Fiscal Quarter', ['Todos'] + quarters)
    if sel_fq != 'Todos':
        df = df[df['Fiscal Quarter'] == sel_fq]
# 2) Forecast Indicator
if 'Forecast Indicator' in df.columns:
    opts_fc = sorted(df['Forecast Indicator'].dropna().unique())
    sel_fc = st.sidebar.selectbox('Forecast Indicator', ['Todos'] + opts_fc)
    if sel_fc != 'Todos':
        df = df[df['Forecast Indicator'] == sel_fc]
# 3) Deal Registration ID
if 'Deal Registration ID' in df.columns:
    dr_ids = sorted(df['Deal Registration ID'].dropna().unique())
    sel_drid = st.sidebar.selectbox('Deal Registration ID', ['Todos'] + dr_ids)
    if sel_drid != 'Todos':
        df = df[df['Deal Registration ID'] == sel_drid]
# 4) Days Since Next Steps Modified (agrupado)
if 'Days Since Next Steps Modified' in df.columns:
    bins = [0, 7, 14, 30, float('inf')]
    labels = ['<=7 dias', '8-14 dias', '15-30 dias', '>30 dias']
    df['DaysGroup'] = pd.cut(df['Days Since Next Steps Modified'], bins=bins, labels=labels, right=True)
...
if 'Licensing Program Type' in df.columns:
    lpt = sorted(df['Licensing Program Type'].dropna().unique())
    sel_lpt = st.sidebar.selectbox('Licensing Program Type', ['Todos'] + lpt)
    if sel_lpt != 'Todos':
        df = df[df['Licensing Program Type'] == sel_lpt]
# 6) Opportunity
if 'Opportunity' in df.columns:
    ops = sorted(df['Opportunity'].dropna().unique())
    sel_op = st.sidebar.selectbox('Opportunity', ['Todos'] + ops)
    if sel_op != 'Todos':
        df = df[df['Opportunity'] == sel_op]
# 7) Account Name
if 'Account Name' in df.columns:
    anames = sorted(df['Account Name'].dropna().unique())
    sel_an = st.sidebar.selectbox('Account Name', ['Todos'] + anames)
    if sel_an != 'Todos':
        df = df[df['Account Name'] == sel_an]
# 8) Account Address: State/Province
if 'Account Address: State/Province' in df.columns:
    states = sorted(df['Account Address: State/Province'].dropna().unique())
    sel_state = st.sidebar.selectbox('Account Address: State/Province', ['Todos'] + states)
    if sel_state != 'Todos':
        df = df[df['Account Address: State/Province'] == sel_state]
# EDU Filter (respeitando todos os filtros acima)
pre_filter_df = df.copy()
edu_choice = st.sidebar.radio('Filtro EDU', ['All', 'EDU', 'Others'], index=0)
if edu_choice == 'EDU':
    df = pre_filter_df[pre_filter_df['Sub Territory'].astype(str).str.contains('EDU', case=False, na=False)]
elif edu_choice == 'Others':
    df = pre_filter_df[~pre_filter_df['Sub Territory'].astype(str).str.contains('EDU', case=False, na=False)]
else:
    df = pre_filter_df

# Totais atualizados após filtros
...
if sel_list:
    ...
    with st.expander(f"🗂 Ficha: {rec.get('Opportunity','')}", expanded=True):
        ...
        for idx, key in enumerate(highlights):
            with cols[idx%3]:
                st.markdown(
                    f"<span style='color:#FFD700'><strong>{key}:</strong> {rec.get(key,'')} </span>", unsafe_allow_html=True
                )
        st.markdown("<hr/>", unsafe_allow_html=True)
        items = [(k,v) for k,v in rec.items() if k not in highlights + ['Forecast Notes','Next Steps']]
        cols2 = st.columns(3)
        for idx,(k,v) in enumerate(items):
            with cols2[idx%3]: st.markdown(f"**{k}:** {v}")
        st.markdown("<hr/>", unsafe_allow_html=True)
