...
# 3) Monta o DataFrame dos selecionados de forma segura
...
# 4) Se não houver seleção, apenas avisa (mas mantém o AgGrid acima)
if commit_df.empty:
    st.warning("⚠️ Selecione pelo menos um Upside Deal acima para editar.")
else:
    # 5) Editor nativo do Streamlit para edição in-place
    st.markdown('---')
    ...
    # 6) Recalcula o Total New ASV
    total_asv = edited_df['Total New ASV'].sum()
    st.header(f"Upside deals to reach the commit — Total New ASV: {total_asv:,.2f}")

    # 7) Botão de download da versão editada
    csv_upside = edited_df.to_csv(index=False).encode('utf-8')
    st.download_button(
