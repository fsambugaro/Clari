...
commit_file = 'committed_deals_persistent.csv'
...
# 1) Tenta carregar commits persistidos
if 'committed_deals' not in st.session_state:
    if os.path.exists(commit_file):
        st.session_state.committed_deals = pd.read_csv(commit_file)
    else:
        # colunas definidas depois de criar commit_disp
        st.session_state.committed_deals = None  # placeholder
...
# Se ainda não inicializou por não existir o CSV, define com colunas corretas
if st.session_state.committed_deals is None:
    st.session_state.committed_deals = pd.DataFrame(columns=commit_disp.columns)

# 3) Botão que limpa apenas a seleção de Upside Deals
if st.button("✔️ Limpar seleção de Upside Deals"):
    st.session_state.upside_grid_counter += 1

# 4) Exibe AgGrid para seleção de Upside Deals
...
# 5) Monta commit_df a partir da seleção
...
if not commit_df.empty:
    ...
    # merge sem perder itens antigos
    existing = st.session_state.committed_deals
    combined = pd.concat([existing, edited_df], ignore_index=True)
    combined = combined.drop_duplicates(subset=['Deal Registration ID'], keep='first')
    st.session_state.committed_deals = combined

    # salva no CSV persistente
    combined.to_csv(commit_file, index=False)

#  7) Editor in-place dos Committed Deals já persistidos
st.markdown('---')
st.subheader('✏️ Editar Committed Deals (você pode alterar ou excluir linhas)')

# exibe o data_editor no lugar do dataframe estático
...
# se houve alteração, atualiza o state e persiste no CSV
...
# 8) Recalcula o Total New ASV após edição/exclusão
...
# 9) Botão de download da versão editada
