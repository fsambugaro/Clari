...
# Region filter: Brazil or Hispanic
regions = ['Todos', 'Brazil', 'Hispanic']
sel_region = st.sidebar.selectbox('Region', regions)
if sel_region != 'Todos':
    df = df[df['Sub Territory'].astype(str).str.contains(sel_region, case=False, na=False)]
# EDU shortcut button: filtra Sub Territory contendo 'EDU'
if st.sidebar.button('EDU'):
    df = df[df['Sub Territory'].astype(str).str.contains('EDU', case=False, na=False)]
# Forecast Indicator
...
exclude = [
    ...
    'Forecast Indicator','Sub Territory','Region','Record Owner','Account Name','Account Name.1',
